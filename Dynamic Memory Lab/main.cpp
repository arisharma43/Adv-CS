...
int main()
{
	ifstream data ("SalesDataDyn.csv");
  ...
  SALESREC* CSV_Pointers[1000];
    ...
    SALESREC* loopPoint;
    loopPoint = (SALESREC*)calloc(1, sizeof(SALESREC));
    loopPoint = new SALESREC;

    loopPoint -> setDate(vals[0]);  
		loopPoint -> setRegion(vals[1]);
		loopPoint -> setRep(vals[2]);
		loopPoint -> setItem(vals[3]);
		loopPoint -> setUnits((int)atoi(vals[4].c_str())); 
		loopPoint -> setUnitCost((float)atof(vals[5].c_str())); 

		loopPoint -> setTotal();
    CSV_Pointers[CSV_WorldSize] = loopPoint;
		CSV_WorldSize++;
